...
if {$mode eq {reg}} {
	...
	vcom -work work -2008 -novopt ../library.vhd
}
...
if {{verilog} eq {verilog}} {
	vlog -work work -novopt -stats=none ../../place_and_router/adder_post.v
} else {
	vcom -work work -2008 -novopt ../../place_and_route/adder_post.v
}
...
if {$mode eq {involution}} {
	...
	vsim work.circuit_TB -t fs -novopt -sdfnoerror -sdfmin c1=../../place_and_route/adder_altered.sdf -GVectorsDir=notUsed -GV_DD=0.80 -GV_TH=0.40
} else {
	vsim work.circuit_tb -t fs -novopt -sdfnoerror -sdfmin c1=../../place_and_route/adder_altered.sdf -GVectorsDir=notUsed   
}

if {$method eq {questa}} {
add wave /circuit_tb/din
add wave /circuit_tb/dout
wave add circuit_tb/c1/Abuf2*
wave add circuit_tb/c1/Cint*
wave add circuit_tb/c1/Sbuf1*
...
vcd add circuit_tb/c1/Abuf2*
vcd add circuit_tb/c1/Cint*
vcd add circuit_tb/c1/Sbuf1*
